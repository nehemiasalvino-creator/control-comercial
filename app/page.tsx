...
function Programaciones({token}:{token:string}) {
  ...
  return <section className="programming workspace-scroll">
    <div className="week-nav sticky-week"><button onClick={()=>setSelectedDate(addDays(weekStart,-7))}>←</button><div>{week.map(date=><button key={date} className={date===selectedDate?"selected":""} onClick={()=>setSelectedDate(date)}><small>{["Dom","Lun","Mar","Mié","Jue","Vie","Sáb"][new Date(`${date}T12:00:00`).getDay()]}</small><b>{date.slice(8,10)}</b><em>{date<today?"Cerrado":programs[date]?.generated?"Programado":"Editable"}</em></button>)}</div><button onClick={()=>setSelectedDate(addDays(weekStart,7))}>→</button></div>
