...
const realWeekValues:Record<string,Record<string,[number,number]>>={
  "2026-08-19":{"EE.SS. Mariscal Sucre":[20000,12000],"E.S. Azari":[20000,12500],"EOSO El Morro":[20000,12500],"EOSO Juana Azurduy":[0,24000],"EOSO María Alejandra":[20000,12000],"EOSO Mesa Verde":[20000,20000],"EOSO Nayler":[20000,24000],"EOSO Oqharikuna SRL":[20000,20000],"EOSO San Antonio":[20000,35000],"EOSO Trébol SRL":[20000,28000],"EESS Ostria Gutiérrez · YPFB":[12000,24000],"EESS El Tejar · YPFB":[12000,12000],"EOSO Aiquile":[4900,24000],"EOSO Murillo · Zudáñez":[12000,12000],"E.S. Pujllay · Tarabuco":[13500,10500],"EESS Serrano · YPFB":[12000,12000]},
...
const dispatch1908:Record<string,[number,number]>={"EE.SS. Mariscal Sucre":[20000,12000],"E.S. Azari":[20000,12500],"EOSO El Morro":[20000,12500],"EOSO Juana Azurduy":[0,24000],"EOSO María Alejandra":[20000,12000],"EOSO Mesa Verde":[20000,20000],"EOSO Nayler":[20000,24000],"EOSO Oqharikuna SRL":[20000,20000],"EOSO San Antonio":[20000,35000],"EOSO Trébol SRL":[20000,28000],"EESS Ostria Gutiérrez · YPFB":[12000,24000],"EESS El Tejar · YPFB":[12000,12000],"EOSO Aiquile":[4900,24000],"EOSO Murillo · Zudáñez":[12000,12000],"E.S. Pujllay · Tarabuco":[13500,10500],"EESS Serrano · YPFB":[12000,12000]};
...
function Programaciones({token}:{token:string}) {
  const today=iso(new Date()),[selectedDate,setSelectedDate]=useState(today),[programs,setPrograms]=useState<Record<string,DayProgram>>(seededPrograms),[notice,setNotice]=useState(""),[dispatchFile,setDispatchFile]=useState<File|null>(null),[parsedDispatch,setParsedDispatch]=useState<{date:string;volumes:Record<string,[number,number]>;orders:number}|null>(null);
  const weekStart=mondayOf(selectedDate),week=Array.from({length:7},(_,i)=>addDays(weekStart,i)),current=programs[selectedDate]||emptyDay(),locked=selectedDate<today;
  const fmt=(value:number)=>new Intl.NumberFormat("es-BO").format(value),setCurrent=(next:DayProgram)=>{if(!locked)setPrograms(all=>({...all,[selectedDate]:next}))};
  useEffect(()=>{loadProgrammingDays(token,week).then(saved=>setPrograms(all=>({...all,...saved}))).catch(error=>setNotice(error.message))},[token,weekStart]);
  const totalD=current.rows.reduce((s,r)=>s+r.programmedDiesel,0),totalG=current.rows.reduce((s,r)=>s+r.programmedGas,0),available=current.stationDiesel+current.stationGas+current.directDiesel+current.directGas;
  const autoVolume=current.rows.reduce((s,r)=>s+(r.manual?0:r.programmedDiesel+r.programmedGas),0),autoPct=Math.round(autoVolume/Math.max(1,totalD+totalG)*100),manualPct=totalD+totalG?100-autoPct:0;
  const stationPct=Math.round((current.stationDiesel+current.stationGas)/Math.max(1,available)*100),directPct=100-stationPct;
  const dispatches=parsedDispatch?.date===selectedDate?parsedDispatch.volumes:selectedDate==="2026-08-19"?dispatch1908:{},dispatchedD=Object.values(dispatches).reduce((s,v)=>s+v[0],0),dispatchedG=Object.values(dispatches).reduce((s,v)=>s+v[1],0),pendingD=Math.max(0,totalD-dispatchedD),pendingG=Math.max(0,totalG-dispatchedG);
  const weekTotals=week.map(date=>{const p=programs[date];return{date,d:p?.rows.reduce((s,r)=>s+r.programmedDiesel,0)||0,g:p?.rows.reduce((s,r)=>s+r.programmedGas,0)||0}}),maxDay=weekTotals.reduce((a,b)=>a.d+a.g>b.d+b.g?a:b,weekTotals[0]),prodeDaily=0;
  const sumDay=(day:DayProgram)=>({d:day.rows.reduce((s,r)=>s+r.programmedDiesel,0),g:day.rows.reduce((s,r)=>s+r.programmedGas,0)});
  const yesterday=addDays(today,-1),todayPlan=programs[today]||emptyDay(),yesterdayPlan=programs[yesterday]||emptyDay(),todaySum=sumDay(todayPlan),yesterdaySum=sumDay(yesterdayPlan);
  const yesterdayDispatch=parsedDispatch?.date===yesterday?parsedDispatch.volumes:yesterday==="2026-08-19"?dispatch1908:{},yesterdayDispatchedD=Object.values(yesterdayDispatch).reduce((s,v)=>s+v[0],0),yesterdayDispatchedG=Object.values(yesterdayDispatch).reduce((s,v)=>s+v[1],0);
  const todayAvailable=todayPlan.stationDiesel+todayPlan.stationGas+todayPlan.directDiesel+todayPlan.directGas,todayStationPct=Math.round((todayPlan.stationDiesel+todayPlan.stationGas)/Math.max(1,todayAvailable)*100),todayDirectPct=100-todayStationPct,todayAutoVolume=todayPlan.rows.reduce((s,r)=>s+(r.manual?0:r.programmedDiesel+r.programmedGas),0),todayAutoPct=Math.round(todayAutoVolume/Math.max(1,todaySum.d+todaySum.g)*100),todayManualPct=todaySum.d+todaySum.g?100-todayAutoPct:0;
  const futureSummary=weekTotals.filter(item=>item.date>today),compareMax=Math.max(1,yesterdaySum.d,yesterdaySum.g,yesterdayDispatchedD,yesterdayDispatchedG);
  function quota(field:keyof Pick<DayProgram,"stationDiesel"|"stationGas"|"directDiesel"|"directGas">,value:string){setCurrent({...current,[field]:Math.max(0,Number(value)||0)})}
  function generate(){
    let remD=current.stationDiesel,remG=current.stationGas;
    const weekday=new Date(`${selectedDate}T12:00:00`).getDay();
    const prior=Object.entries(programs).filter(([date])=>date<selectedDate&&new Date(`${date}T12:00:00`).getDay()===weekday).map(([,day])=>day);
    const rows=current.rows.map((row,index)=>{
      ...
      const history=prior.map(day=>day.rows.find(item=>item.name===row.name)).filter(Boolean) as ProgramRow[];
      const shouldRun=history.length?history.some(item=>item.programmedDiesel+item.programmedGas>0):(index+weekday)%Math.max(1,Math.round(row.compartments.reduce((a,b)=>a+b,0)/Math.max(1,row.diesel+row.gas)))===0;
      if(!shouldRun)return{...row,programmedDiesel:0,programmedGas:0,autoDiesel:0,autoGas:0,manual:false};
      const targetD=history.length?history.reduce((s,r)=>s+r.programmedDiesel,0)/history.length:row.diesel,targetG=history.length?history.reduce((s,r)=>s+r.programmedGas,0)/history.length:row.gas;
      let d=0,g=0;for(const capacity of row.compartments){const favorD=targetD/Math.max(1,targetD+targetG)>=d/Math.max(1,d+g);if(favorD&&remD>=capacity){d+=capacity;remD-=capacity}else if(remG>=capacity){g+=capacity;remG-=capacity}else if(remD>=capacity){d+=capacity;remD-=capacity}}
      return{...row,programmedDiesel:d,programmedGas:g,autoDiesel:d,autoGas:g,manual:false};
    });
    const next={...current,rows,generated:true};setCurrent(next);saveProgrammingDay(token,selectedDate,next,100).then(()=>setNotice("Programación automática guardada.")).catch(error=>setNotice(error.message));
  }
  function edit(index:number,field:"programmedDiesel"|"programmedGas",value:string){const next={...current,rows:current.rows.map((row,i)=>i===index?{...row,[field]:Math.max(0,Number(value)||0),manual:true}:row)};setCurrent(next);const volume=next.rows.reduce((s,r)=>s+r.programmedDiesel+r.programmedGas,0),auto=next.rows.reduce((s,r)=>s+(r.manual?0:r.programmedDiesel+r.programmedGas),0);saveProgrammingDay(token,selectedDate,next,Math.round(auto/Math.max(1,volume)*100)).then(()=>setNotice("Ajuste guardado.")).catch(error=>setNotice(error.message))}
  async function reconcile(){if(!dispatchFile)return setNotice("Seleccione el PDF de despachos.");try{const parsed=await parseDispatchPdf(dispatchFile);setParsedDispatch(parsed);setSelectedDate(parsed.date);setNotice(`${parsed.orders} órdenes conciliadas para ${parsed.date.split("-").reverse().join("/")}.`)}catch{setNotice("No se pudo leer el PDF. Verifique que corresponda al formato Despachos de Cisternas.")}}
  return <section className="programming workspace-scroll">
    <div className="week-nav sticky-week"><button onClick={()=>setSelectedDate(addDays(weekStart,-7))}>←</button><div>{week.map(date=><button key={date} className={`${date===selectedDate?"selected ":""}${date===today?"current-day":""}`.trim()} onClick={()=>setSelectedDate(date)}><small>{["Dom","Lun","Mar","Mié","Jue","Vie","Sáb"][new Date(`${date}T12:00:00`).getDay()]}</small><b>{date.slice(8,10)}</b><strong>{date===today?"HOY":""}</strong><em>{date<today?"Cerrado":programs[date]?.generated?"Programado":"Editable"}</em></button>)}</div><button onClick={()=>setSelectedDate(addDays(weekStart,7))}>→</button></div>
    <div className="scroll-content"><div className="allocation-split"><span>Distribución disponible</span><div><i style={{width:`${stationPct}%`}}/><u style={{width:`${directPct}%`}}/></div><b>{stationPct}% estaciones</b><b>{directPct}% directos / GRACO</b></div>
    <div className="program-toolbar compact-toolbar"><div className="quota-grid"><label>Diésel · estaciones<input disabled={locked} type="number" value={current.stationDiesel} onChange={e=>quota("stationDiesel",e.target.value)}/></label><label>Gasolina · estaciones<input disabled={locked} type="number" value={current.stationGas} onChange={e=>quota("stationGas",e.target.value)}/></label><label>Diésel · directos/GRACO<input disabled={locked} type="number" value={current.directDiesel} onChange={e=>quota("directDiesel",e.target.value)}/></label><label>Gasolina · directos/GRACO<input disabled={locked} type="number" value={current.directGas} onChange={e=>quota("directGas",e.target.value)}/></label></div><div className="program-buttons"><button disabled={locked} className="primary small" onClick={generate}>Programar día</button><button className="secondary" onClick={()=>window.print()}>Publicación gráfica</button></div></div>
    {notice&&<div className="admin-message">{notice}</div>}{locked&&<div className="locked-note">Cerrado a las 00:00. Disponible únicamente para consulta, conciliación e impresión.</div>}
    <section className="kpi-grid"><Kpi label="MÁXIMO DE LA SEMANA" value={`${fmt(maxDay.d+maxDay.g)} L`} note={prodeDaily?"Comparado con PRODE":"Línea PRODE pendiente de carga"}/><Kpi label="DISPONIBLE PARA EL DÍA" value={`${fmt(available)} L`} note={`${fmt(totalD+totalG)} L actualmente asignados`} tone="green"/><Kpi label="ASIGNACIÓN AUTOMÁTICA" value={`${autoPct}%`} note="Aprende del mismo día de semanas anteriores" tone="violet"/><Kpi label="AJUSTE MANUAL" value={`${manualPct}%`} note="Debe disminuir al aprender el patrón" tone="amber"/></section>
    <article className="card weekly-graphic"><CardHead title="Programación semanal y línea PRODE" subtitle={prodeDaily?`PRODE diario ${fmt(prodeDaily)} L`:"La referencia aparecerá al cargar el PRODE"}/><div className="week-bars improved">{weekTotals.map(item=><div key={item.date}><span>{fmt(item.d+item.g)}</span><i><u style={{height:`${item.d/Math.max(1,maxDay.d+maxDay.g)*130}px`}}/><em style={{height:`${item.g/Math.max(1,maxDay.d+maxDay.g)*130}px`}}/></i><b>{item.date.slice(8,10)}</b><small>D {fmt(item.d)} · G {fmt(item.g)}</small></div>)}</div></article>
    <article className="card week-table"><CardHead title="Tabla de programación semanal" subtitle="Pasado, hoy y futuro identificados por color"/><div className="program-table"><table><thead><tr><th>Cliente</th>{week.map(date=><th key={date} className={date<today?"past":date===today?"today":"future"}>{date.slice(8,10)}<br/>{date<today?"Pasado":date===today?"Hoy":"Futuro"}</th>)}</tr></thead><tbody>{programmingClients.map(client=><tr key={client.name}><td>{client.name}</td>{week.map(date=>{const row=programs[date]?.rows.find(item=>item.name===client.name);return <td key={date} className={date<today?"past":date===today?"today":"future"}><b>D {fmt(row?.programmedDiesel||0)}</b><small>G {fmt(row?.programmedGas||0)}</small></td>})}</tr>)}</tbody></table></div></article>
    <article className="card program-report"><header className="publication-head"><div><small>YPFB · DISTRITO COMERCIAL CHUQUISACA</small><h2>Programación de combustibles</h2><p>{selectedDate.split("-").reverse().join("/")} · Semana {weekStart.split("-").reverse().join("/")} al {week[6].split("-").reverse().join("/")}</p></div><span>{locked?"Cerrado":"Borrador editable"}</span></header><div className="program-table compact"><table><thead><tr><th>Cliente / estación</th><th>Tipo</th><th>Compartimientos</th><th>Diésel</th><th>Gasolina</th><th>Origen</th></tr></thead><tbody>{current.rows.map((row,index)=><tr key={row.name}><td><strong>{row.name}</strong></td><td>{row.category==="station"?"EESS":"Directo / GRACO"}</td><td>{row.compartments.length?row.compartments.map(fmt).join(" + "):"Manual"}</td><td><input disabled={locked} className="programmed" type="number" value={row.programmedDiesel||""} onChange={e=>edit(index,"programmedDiesel",e.target.value)}/></td><td><input disabled={locked} className="programmed" type="number" value={row.programmedGas||""} onChange={e=>edit(index,"programmedGas",e.target.value)}/></td><td><span className={row.manual?"origin manual":"origin auto"}>{row.manual?"Manual":"Automático"}</span></td></tr>)}</tbody><tfoot><tr><td>TOTAL</td><td/><td/><td>{fmt(totalD)}</td><td>{fmt(totalG)}</td><td>{autoPct}% / {manualPct}%</td></tr></tfoot></table></div></article>
    <article className="card dispatch-control"><CardHead title="Conciliación de despachos" subtitle="Carga diaria y seguimiento de pendientes hasta completar"/><div className="dispatch-upload"><label>Subir despacho del día anterior<input type="file" accept=".pdf" onChange={e=>setDispatchFile(e.target.files?.[0]||null)}/></label><div><b>{dispatchFile?.name||"Despachos Cisternas 19-08-2026.pdf"}</b><span>{dispatchFile?"PDF listo para validar":"Ejemplo real analizado"}</span></div><button className="primary small" onClick={reconcile}>Validar y conciliar</button></div><div className="reconcile-kpis"><div><span>Programado</span><b>{fmt(totalD+totalG)} L</b></div><div><span>Despachado</span><b>{fmt(dispatchedD+dispatchedG)} L</b></div><div><span>Pendiente diésel</span><b>{fmt(pendingD)} L</b></div><div><span>Pendiente gasolina</span><b>{fmt(pendingG)} L</b></div></div>{Object.keys(dispatches).length>0&&<div className="reconcile-ok">Conciliación {selectedDate.split("-").reverse().join("/")}: {fmt(dispatchedD)} L DO y {fmt(dispatchedG)} L GE identificados por cliente y orden.</div>}</article>
    <section className="print-publication"><header><div><small>YPFB · DISTRITO COMERCIAL CHUQUISACA</small><h1>Reporte operativo de programación</h1><p>Zona Comercial Sucre · Emitido el {today.split("-").reverse().join("/")}</p></div><b>CONTROL COMERCIAL</b></header><div className="print-kpis"><div><span>PROGRAMADO HOY</span><strong>{fmt(todaySum.d+todaySum.g)} L</strong><small>DO {fmt(todaySum.d)} · GE {fmt(todaySum.g)}</small></div><div><span>DESPACHADO AYER</span><strong>{fmt(yesterdayDispatchedD+yesterdayDispatchedG)} L</strong><small>DO {fmt(yesterdayDispatchedD)} · GE {fmt(yesterdayDispatchedG)}</small></div><div><span>PENDIENTE DE AYER</span><strong>{fmt(Math.max(0,yesterdaySum.d+yesterdaySum.g-yesterdayDispatchedD-yesterdayDispatchedG))} L</strong><small>Seguimiento hasta completar</small></div><div><span>FUTURO PROGRAMADO</span><strong>{fmt(futureSummary.reduce((s,r)=>s+r.d+r.g,0))} L</strong><small>{futureSummary.filter(r=>r.d+r.g>0).length} días con asignación</small></div></div><div className="print-panels"><article><h2>Programado vs. despachado de ayer</h2>{[["Diésel",yesterdaySum.d,yesterdayDispatchedD],["Gasolina",yesterdaySum.g,yesterdayDispatchedG]].map(([label,planned,sent])=><div className="compare-row" key={String(label)}><b>{label}</b><span><i style={{width:`${Number(planned)/compareMax*100}%`}}/><em style={{width:`${Number(sent)/compareMax*100}%`}}/></span><small>Programado {fmt(Number(planned))} · Despachado {fmt(Number(sent))}</small></div>)}<footer><i/> Programado <em/> Despachado</footer></article><article><h2>Transparencia de la programación de hoy</h2><div className="percentage"><span><i style={{width:`${todayStationPct}%`}}/><em style={{width:`${todayDirectPct}%`}}/></span><b>{todayStationPct}% estaciones</b><b>{todayDirectPct}% directos / GRACO</b></div><div className="percentage auto"><span><i style={{width:`${todayAutoPct}%`}}/><em style={{width:`${todayManualPct}%`}}/></span><b>{todayAutoPct}% automático</b><b>{todayManualPct}% ajuste manual</b></div><p>La programación combina criterios estadísticos, capacidades completas de cisternas y ajustes operativos auditables.</p></article></div><article className="print-section"><h2>Programación de hoy por cliente</h2><table><thead><tr><th>Cliente / estación</th><th>Clasificación</th><th>Diésel</th><th>Gasolina</th><th>Origen</th></tr></thead><tbody>{todayPlan.rows.filter(r=>r.programmedDiesel+r.programmedGas>0).map(row=><tr key={row.name}><td>{row.name}</td><td>{row.category==="station"?"EESS":"Directo / GRACO"}</td><td>{fmt(row.programmedDiesel)} L</td><td>{fmt(row.programmedGas)} L</td><td>{row.manual?"Ajuste manual":"Automático"}</td></tr>)}</tbody><tfoot><tr><td colSpan={2}>TOTAL HOY</td><td>{fmt(todaySum.d)} L</td><td>{fmt(todaySum.g)} L</td><td>{todayAutoPct}% automático</td></tr></tfoot></table></article><article className="print-section future-print"><h2>Programación futura de la semana</h2><div>{futureSummary.map(item=><section key={item.date} className={item.d+item.g?"has-program":"empty"}><small>{["Domingo","Lunes","Martes","Miércoles","Jueves","Viernes","Sábado"][new Date(`${item.date}T12:00:00`).getDay()]}</small><b>{item.date.slice(8,10)}</b><strong>{fmt(item.d+item.g)} L</strong><span>DO {fmt(item.d)} · GE {fmt(item.g)}</span></section>)}</div></article><footer className="print-note">Reporte generado por Control Comercial · Programación trazable y sujeta a conciliación con despachos reales.</footer></section></div>
  </section>;
}
...
function ProgramacionesLegacy({token}:{token:string}) {
  ...
  const availableTotal=current.stationDiesel+current.stationGas+current.directDiesel+current.directGas,stationShare=Math.round((current.stationDiesel+current.stationGas)/Math.max(1,availableTotal)*100),directShare=100-stationShare;
  const dispatches=selectedDate==="2026-08-19"?dispatch1908:{},dispatchedD=Object.values(dispatches).reduce((s,v)=>s+v[0],0),dispatchedG=Object.values(dispatches).reduce((s,v)=>s+v[1],0),pendingD=Math.max(0,totalD-dispatchedD),pendingG=Math.max(0,totalG-dispatchedG);
